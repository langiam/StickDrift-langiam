...
const SearchPage: React.FC = () => {
  ...
  // 1) Fetch the full "me" object (including following array)
  const { data: meData, loading: meLoading } = useQuery(QUERY_ME);

  // 2) Lazy query to search other profiles
  const [searchProfiles, { data, loading: searchLoading }] = useLazyQuery(
    QUERY_SEARCH_PROFILE
  );
  ...
  const me = meData?.me;
  const results: any[] = data?.searchProfile || [];
